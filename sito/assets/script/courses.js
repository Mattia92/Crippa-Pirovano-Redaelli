...
function ready(){
    ...
    $.ajax({
        ...
        success: function(response) {
          ...
          /*  var el = courses[i].presentation;
            arrLi.eq(i).html(el);
               arrLi.eq(i).hover(function() {
               		$(this).addClass('show');
               });*/
               
               $('.Title').eq(k).text(courses[i].title.charAt(0));
               //$('.shortDescription').eq(k).text(courses[i].title);
    ...
    $(this).wrapInner('<a href="http://frankgym.altervista.org/pages/course.html?idCourse='+courses[i].id+'">'+courses[i].title+"<br>"+'</a>')});
               ...
               {
                //$('.shortDescription').eq(k).append("<br>"+courses[j].title);
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            for(i=0;i<courses.length;i++) {
               ...
               {
               //$('#works_2 ul li a p.shortDescription').eq(2).append(courses[i].title+"<br>");
               $('#works_2 ul li a p.shortDescription').eq(2).append(function(){
    $(this).wrapInner('<a href="http://frankgym.altervista.org/pages/course.html?idCourse='+courses[i].id+'">'+courses[i].title+"<br>"+'</a>')});
               }
               if(courses[i].level.charAt(0) == "m")
               {
               //$('#works_2 ul li a p.shortDescription').eq(1).append(courses[i].title+"<br>");
               $('#works_2 ul li a p.shortDescription').eq(1).append(function(){
    $(this).wrapInner('<a href="http://frankgym.altervista.org/pages/course.html?idCourse='+courses[i].id+'">'+courses[i].title+"<br>"+'</a>')});
               }
               if(courses[i].level.charAt(0) == "l")
               {
               //$('#works_2 ul li a p.shortDescription').eq(0).append(courses[i].title+"<br>");
